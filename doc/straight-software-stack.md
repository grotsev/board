...
Атрибут '''runOnChange:true''' указывает, что нужно заново применить набор изменений, если текст функции изменился, автор '''auto''', идентификатор совпадает с именем функции. Так как функции подменяются, то они должны быть объявлены '''create or replace function'''.
